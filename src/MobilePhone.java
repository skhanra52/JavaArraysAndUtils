...
public class MobilePhone {
    ...
    public boolean updateContact(Contact contact, Contact contactToBeUpdated){
        int index =  findContact(contact);
        if(index>=0){
            myContacts.set(index,contactToBeUpdated);
            return true;
        }else{
            return false;
        }
    ...
    public Contact queryContact(String contactSearch){
        int index = findContact(contactSearch);
        if(index>=0){
    ...
    public void printContacts(){
        ...
        for (int i = 0; i < myContacts.size(); i++) {
            ...
            String nameChar = contact.getName().trim();
            String name = nameChar.substring(0,1).toUpperCase() + nameChar.substring(1).toLowerCase();
            // System.out.println((i + 1) + ". " + name + " -> " + contact.getPhoneNumber());
            System.out.println(String.format("%d. %s -> %s", i + 1, name, contact.getPhoneNumber()));

        }
    }

    public boolean removeContact(Contact removeContact){
        int index =  findContact(removeContact);
        boolean flag = false;
    ...
    private int findContact(Contact contactToBeFound) {
        String nameToSearch = contactToBeFound.getName().trim();
        for (int i = 0; i < myContacts.size(); i++) {
            Contact contact = myContacts.get(i);
            if (contact.getName().trim().equalsIgnoreCase(nameToSearch)) {
    ...
    private int findContact(String contactToBeFind) {
        for (int i = 0; i < myContacts.size(); i++) {
            if (myContacts.get(i).getName().trim().equalsIgnoreCase(contactToBeFind.trim())) {
                return i; // Return as soon as a match is found
            }
        }
        return -1; // Not found
    }
